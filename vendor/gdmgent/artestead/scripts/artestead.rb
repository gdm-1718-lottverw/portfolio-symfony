class Artestead
    def Artestead.configure(config, settings)
        # Set The VM Provider
        ...
        config.vm.hostname = settings["hostname"] ||="artestead"

        # Configure A Private Network IP
        config.vm.network :private_network, ip: settings["ip"] ||= "192.168.10.10", host_ip: "127.0.0.1"
        ...
        if settings.include? 'folders'
            settings["folders"].each do |folder|
                if File.exists? File.expand_path(folder["map"])
                    ...
                    config.vm.provision "shell" do |s|
                        s.inline = ">&2 echo \"Unable to mount one of your folders. Please check your folders in Artestead.yaml\""
                    end
                end
            end
        end
        ...
        if settings.has_key?("variables")
            settings["variables"].each do |var|
                ...
                config.vm.provision "shell" do |s|
                    s.inline = "echo \"\n# Set Artestead Environment Variable\nexport $1=$2\" >> /home/vagrant/.profile"
                    s.args = [var["key"], var["value"]]
                end
            end
